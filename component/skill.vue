<script setup lang="ts">

</script>

<template>
  <UCard class="h-full w-full">
    <p class="w-full flex justify-center text-2xl">Compétences</p>


    <div class="flex justify-center">
      <UDivider class="p-4" orientation="vertical"/>
      <!-- Mobile -->
      <div>
        <UDivider class="p-2" label="Mobile"/>
        <div class="flex gap-4">
          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <path fill="#FF6D00"
                    d="M6,10c0-2.209,1.791-4,4-4h28c2.209,0,4,1.791,4,4v28c0,2.208-1.791,4-4,4H10c-2.209,0-4-1.792-4-4V10z"></path>
              <path fill="#E64A19"
                    d="M6,10v11.967l4,4c0,0,11.153,12.881,20,4.033s-3.981-19-3.981-19l-5.005-5H10C7.791,6,6,7.79,6,10z"></path>
              <path fill="#FFF"
                    d="M26.019,11c11.429,7.698,7.732,16.187,7.732,16.187s3.25,3.634,1.936,6.813c0,0-1.34-2.226-3.588-2.226c-2.166,0-3.44,2.226-7.8,2.226C14.591,34,10,25.967,10,25.967c8.747,5.7,14.719,1.663,14.719,1.663C20.778,25.364,12,14,12,14c7.299,6.155,11,8,11,8c-1.882-1.542-8-9-8-9c4.224,4.238,13.307,10.378,13.307,10.378C30.691,16.834,26.019,11,26.019,11z"></path>
            </svg>
            <p class="flex justify-center">Swift</p>
          </div>

          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <polygon fill="#0d91d6" points="24,6 17.99,12.35 11.97,18.69 6,25 6,6"></polygon>
              <polygon fill="#f88909"
                       points="24,6 23.96,6 17.97,12.33 11.96,18.68 6,24.97 6,25 6,25.09 6,42 14.69,33.31 20.87,27.13 27.06,20.94 33.25,14.75 42,6"></polygon>
              <polygon fill="#c757bc" points="14.69,33.31 6,42 6,25.09"></polygon>
              <polygon fill="#d3638f" points="20.87,27.13 14.69,33.31 6,25.09 6,25 11.97,18.69"></polygon>
              <polygon fill="#e07063" points="27.06,20.94 20.87,27.13 11.97,18.69 17.99,12.35"></polygon>
              <polygon fill="#ec7d36" points="33.25,14.75 27.06,20.94 17.99,12.35 24,6"></polygon>
              <polygon fill="#f88909" points="42,6 33.25,14.75 24,6"></polygon>
              <polygon fill="#0095d5" points="11.51,19.15 7.99,22.88 6,21 6,13.94"></polygon>
              <polygon fill="#2b88da" points="15.03,15.42 11.51,19.15 6,13.94 6,6.87"></polygon>
              <polygon fill="#557bde" points="18.56,11.7 15.03,15.42 6,6.87 6,6 12.53,6"></polygon>
              <polygon fill="#806ee3" points="22.08,7.97 18.56,11.7 12.53,6 24,6"></polygon>
              <polygon fill="#0095d5" points="23,25 6,42 9.35,42 17.52,42 40,42"></polygon>
              <polygon fill="#2b88da" points="25.68,42 17.52,42 11.76,36.25 15.84,32.17"></polygon>
              <polygon fill="#557bde" points="33.86,42 25.68,42 15.84,32.17 19.92,28.09"></polygon>
              <polygon fill="#806ee3" points="42,42 33.86,42 19.92,28.09 24,24"></polygon>
            </svg>

            <p class="flex justify-center">Kotlin</p>
          </div>

          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <polygon fill="#40c4ff" points="26,4 6,24 12,30 38,4"></polygon>
              <polygon fill="#40c4ff" points="38,22 27,33 21,27 26,22"></polygon>
              <rect width="8.485" height="8.485" x="16.757" y="28.757" fill="#03a9f4"
                    transform="rotate(-45.001 21 33)"></rect>
              <polygon fill="#01579b" points="38,44 26,44 21,39 27,33"></polygon>
              <polygon fill="#084994" points="21,39 30,36 27,33"></polygon>
            </svg>
            <p class="flex justify-center">Flutter</p>
          </div>
        </div>
        <UDivider class="p-4"/>
      </div>
      <UDivider class="p-4" orientation="vertical"/>

      <!-- Web -->
      <div>
        <UDivider class="p-2" label="Web"/>
        <div class="flex gap-4">
          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <path fill="#bdbdbd" d="M23.933 2L3 9.285 6.308 36.408 23.955 46 41.693 36.278 45 9.156z"></path>
              <path fill="#b71c1c" d="M42.818 10.527L24 4.135 24 43.695 39.832 35.017z"></path>
              <path fill="#dd2c00" d="M23.941 4.115L5.181 10.644 8.168 35.143 23.951 43.721 24 43.695 24 4.135z"></path>
              <path fill="#bdbdbd" d="M24 5.996L24 15.504 32.578 34 36.987 34z"></path>
              <path fill="#eee" d="M11.013 34L15.422 34 24 15.504 24 5.996z"></path>
              <path fill="#bdbdbd" d="M24 24H30V28H24z"></path>
              <path fill="#eee" d="M18 24H24V28H18z"></path>
            </svg>
            <p class="flex justify-center">Angular</p>
          </div>

          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <path fill="#80deea"
                    d="M24,34C11.1,34,1,29.6,1,24c0-5.6,10.1-10,23-10c12.9,0,23,4.4,23,10C47,29.6,36.9,34,24,34z M24,16	c-12.6,0-21,4.1-21,8c0,3.9,8.4,8,21,8s21-4.1,21-8C45,20.1,36.6,16,24,16z"></path>
              <path fill="#80deea"
                    d="M15.1,44.6c-1,0-1.8-0.2-2.6-0.7C7.6,41.1,8.9,30.2,15.3,19l0,0c3-5.2,6.7-9.6,10.3-12.4c3.9-3,7.4-3.9,9.8-2.5	c2.5,1.4,3.4,4.9,2.8,9.8c-0.6,4.6-2.6,10-5.6,15.2c-3,5.2-6.7,9.6-10.3,12.4C19.7,43.5,17.2,44.6,15.1,44.6z M32.9,5.4	c-1.6,0-3.7,0.9-6,2.7c-3.4,2.7-6.9,6.9-9.8,11.9l0,0c-6.3,10.9-6.9,20.3-3.6,22.2c1.7,1,4.5,0.1,7.6-2.3c3.4-2.7,6.9-6.9,9.8-11.9	c2.9-5,4.8-10.1,5.4-14.4c0.5-4-0.1-6.8-1.8-7.8C34,5.6,33.5,5.4,32.9,5.4z"></path>
              <path fill="#80deea"
                    d="M33,44.6c-5,0-12.2-6.1-17.6-15.6C8.9,17.8,7.6,6.9,12.5,4.1l0,0C17.4,1.3,26.2,7.8,32.7,19	c3,5.2,5,10.6,5.6,15.2c0.7,4.9-0.3,8.3-2.8,9.8C34.7,44.4,33.9,44.6,33,44.6z M13.5,5.8c-3.3,1.9-2.7,11.3,3.6,22.2	c6.3,10.9,14.1,16.1,17.4,14.2c1.7-1,2.3-3.8,1.8-7.8c-0.6-4.3-2.5-9.4-5.4-14.4C24.6,9.1,16.8,3.9,13.5,5.8L13.5,5.8z"></path>
              <circle cx="24" cy="24" r="4" fill="#80deea"></circle>
            </svg>
            <p class="flex justify-center">React</p>
          </div>

          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <polygon fill="#81c784" points="23.987,17 18.734,8 2.974,8 23.987,44 45,8 29.24,8"></polygon>
              <polygon fill="#455a64" points="29.24,8 23.987,17 18.734,8 11.146,8 23.987,30 36.828,8"></polygon>
            </svg>
            <p class="flex justify-center">Vue</p>
          </div>
        </div>
        <UDivider class="p-4"/>
      </div>
      <UDivider class="p-4" orientation="vertical"/>

      <!-- Back end -->
      <div>
        <UDivider class="p-2" label="Autre"/>
        <div class="flex gap-4">
          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <path fill="#F44336"
                    d="M23.65,24.898c-0.998-1.609-1.722-2.943-2.725-5.455C19.229,15.2,31.24,11.366,26.37,3.999c2.111,5.089-7.577,8.235-8.477,12.473C17.07,20.37,23.645,24.898,23.65,24.898z"></path>
              <path fill="#F44336"
                    d="M23.878,17.27c-0.192,2.516,2.229,3.857,2.299,5.695c0.056,1.496-1.447,2.743-1.447,2.743s2.728-0.536,3.579-2.818c0.945-2.534-1.834-4.269-1.548-6.298c0.267-1.938,6.031-5.543,6.031-5.543S24.311,11.611,23.878,17.27z"></path>
              <g>
                <path fill="#1565C0"
                      d="M32.084 25.055c1.754-.394 3.233.723 3.233 2.01 0 2.901-4.021 5.643-4.021 5.643s6.225-.742 6.225-5.505C37.521 24.053 34.464 23.266 32.084 25.055zM29.129 27.395c0 0 1.941-1.383 2.458-1.902-4.763 1.011-15.638 1.147-15.638.269 0-.809 3.507-1.638 3.507-1.638s-7.773-.112-7.773 2.181C11.683 28.695 21.858 28.866 29.129 27.395z"></path>
                <path fill="#1565C0"
                      d="M27.935,29.571c-4.509,1.499-12.814,1.02-10.354-0.993c-1.198,0-2.974,0.963-2.974,1.889c0,1.857,8.982,3.291,15.63,0.572L27.935,29.571z"></path>
                <path fill="#1565C0"
                      d="M18.686,32.739c-1.636,0-2.695,1.054-2.695,1.822c0,2.391,9.76,2.632,13.627,0.205l-2.458-1.632C24.271,34.404,17.014,34.579,18.686,32.739z"></path>
                <path fill="#1565C0"
                      d="M36.281,36.632c0-0.936-1.055-1.377-1.433-1.588c2.228,5.373-22.317,4.956-22.317,1.784c0-0.721,1.807-1.427,3.477-1.093l-1.42-0.839C11.26,34.374,9,35.837,9,37.017C9,42.52,36.281,42.255,36.281,36.632z"></path>
                <path fill="#1565C0"
                      d="M39,38.604c-4.146,4.095-14.659,5.587-25.231,3.057C24.341,46.164,38.95,43.628,39,38.604z"></path>
              </g>
            </svg>
            <p class="flex justify-center">Java</p>
          </div>

          <div>
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="48" height="48" viewBox="0 0 48 48">
              <circle cx="24" cy="25" r="14" fill="#1565c0"></circle>
              <path
                  d="M22.531,44.942l-0.182-0.015c-1.654-0.125-3.365-0.485-5-1.061l0.664-1.887c1.471,0.518,3.011,0.842,4.578,0.964 L22.531,44.942z M27.665,44.666l-0.363-1.967c1.54-0.285,3.041-0.775,4.461-1.455l0.865,1.803 C31.049,43.804,29.379,44.349,27.665,44.666z M12.695,41.502c-1.438-0.987-2.745-2.16-3.883-3.488l1.52-1.301 c1.023,1.195,2.199,2.252,3.494,3.141L12.695,41.502z M37,40.199l-1.301-1.52c1.196-1.023,2.254-2.198,3.144-3.492l1.648,1.133 C39.503,37.758,38.328,39.062,37,40.199z M5.961,33.646c-0.758-1.577-1.304-3.247-1.624-4.962l1.967-0.366 c0.287,1.542,0.778,3.043,1.46,4.461L5.961,33.646z M42.861,31.669l-1.887-0.664c0.521-1.478,0.847-3.024,0.97-4.597l1.003,0.005 l0.997,0.08l-0.006,0.075C43.802,28.311,43.439,30.028,42.861,31.669z M6.056,23.592L4.059,23.47l0.004-0.04 c0.135-1.731,0.493-3.437,1.064-5.064l1.887,0.662C6.501,20.493,6.178,22.028,6.056,23.592z M41.69,21.649 c-0.292-1.543-0.786-3.043-1.47-4.458l1.801-0.87c0.76,1.574,1.31,3.242,1.634,4.958L41.69,21.649z M9.138,14.84l-1.65-1.13 c0.985-1.438,2.157-2.746,3.484-3.885l1.303,1.518C11.079,12.368,10.024,13.545,9.138,14.84z M37.646,13.261 c-1.027-1.194-2.205-2.248-3.501-3.134l1.129-1.651c1.439,0.984,2.747,2.155,3.888,3.48L37.646,13.261z M16.205,8.772L15.338,6.97 c1.575-0.759,3.243-1.307,4.958-1.628l0.369,1.966C19.123,7.597,17.622,8.09,16.205,8.772z M29.955,8.008 c-1.461-0.511-2.99-0.831-4.547-0.953l0.023-1.001l0.079-0.997l-0.079,0.997l0.11-0.995c1.75,0.137,3.45,0.494,5.073,1.061 L29.955,8.008z"></path>
              <circle cx="28.5" cy="18.5" r="4.5" fill="#fff"></circle>
              <circle cx="37.5" cy="9.5" r="4.5" fill="#1565c0"></circle>
              <polygon fill="#fff" points="19,33 14,33 14,24 15,24 15,32 19,32"></polygon>
              <path fill="#fff"
                    d="M22.504,32.969c-1.591,0-2.504-0.932-2.504-2.556V26h1v4.413c0,1.076,0.464,1.556,1.504,1.556 c1.124,0,2.496-0.844,2.496-1.829V26h1v4.14C26,31.733,24.121,32.969,22.504,32.969z"></path>
              <path fill="#fff"
                    d="M33,33h-1v-5.005c0-0.992-0.525-1.495-1.562-1.495c-0.66,0-1.643,0.274-1.643,1.03h-1 c0-1.333,1.329-2.03,2.643-2.03c1.581,0,2.562,0.956,2.562,2.495V33z"></path>
              <rect width="1" height="3" x="25" y="30" fill="#fff"></rect>
              <path fill="#fff"
                    d="M29.755,33.053c-1.285,0-2.255-0.99-2.255-2.303c0-1.537,1.625-2.012,2.932-2.395 c0.673-0.197,1.368-0.4,1.648-0.685l0.713,0.701c-0.463,0.47-1.249,0.7-2.081,0.943c-1.336,0.391-2.212,0.702-2.212,1.435 c0,0.755,0.527,1.303,1.255,1.303c1.059,0,1.765-0.369,2.291-1.196l0.844,0.536C32.18,32.51,31.154,33.053,29.755,33.053z"></path>
            </svg>
            <p class="flex justify-center">Lua</p>
          </div>
        </div>
        <UDivider class="p-4"/>
      </div>
      <UDivider class="p-4" orientation="vertical"/>
    </div>

  </UCard>
</template>

<style scoped>

</style>